...
// public
    ...
    get(id: EntityId): T | null {
        if (id >= this.m_sparse.length) return null;
        const index = this.m_sparse[id]!;
        if (index === k_poolInvalid) return null;
        return this.m_denseComponents[index]!;
    }
    ...
    size(): number     { return this.m_denseEntities.length; }
    entityAt(i: number): EntityId { return this.m_denseEntities[i]!; }
    at(i: number): T               { return this.m_denseComponents[i]!; }
