...
// public
    ...
    addComponent<T extends Component>(id: EntityId, component: T): T {
        if (!this.m_componentPools.has(component.sType)) {
            this.m_componentPools.set(component.sType, new ComponentPool<T>());
        }

        const result = (this.m_componentPools.get(component.sType) as ComponentPool<T>).emplace(id, component);
    ...
    getComponent<T extends Component>(id: EntityId, sType: string): T | null {
        return (this.m_componentPools.get(sType) as ComponentPool<T> | undefined)?.get(id) ?? null;
    }

    removeComponent(id: EntityId, sType: string): void {
        if (!this.m_componentPools.get(sType)?.has(id)) return;

        this.m_componentPools.get(sType)!.removeById(id);
    ...
    getPool<T extends Component>(sType: string): ComponentPool<T> | null {
        return (this.m_componentPools.get(sType) as ComponentPool<T>) ?? null;
    }

    view<A extends Component>(a: string): View<[A]>;
    view<A extends Component, B extends Component>(a: string, b: string): View<[A, B]>;
    view<A extends Component, B extends Component, C extends Component>(a: string, b: string, c: string): View<[A, B, C]>;
    view<A extends Component, B extends Component, C extends Component, D extends Component>(a: string, b: string, c: string, d: string): View<[A, B, C, D]>;
    view(...componentNames: string[]): View<Component[]> {
        return new View(this, ...componentNames);
    }
...
// private
    ...
    private m_componentPools: Map<string, IComponentPool> = new Map();
}
